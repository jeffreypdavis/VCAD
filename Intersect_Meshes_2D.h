 ...
 * 
 * File:   Intersect_Meshes_2D.h
 * Author: Jeffrey Davis
 */

#ifndef INTERSECT_MESHES_2D_H
#define INTERSECT_MESHES_2D_H
...
namespace VCAD_lib
{
    ...
    private:
        ...
        struct Intersect_Point {
            shared_ptr<Point_2D> pt; // the intersection point
             ...
             */
            Intersect_Point(const shared_ptr<Point_2D> i_point, const Location f1_location, 
        ...
        public:
             ...
             */
            const bool validate(const Facet_2D& facet1, const Facet_2D& facet2);
        private:
             ...
             */
            const bool matches(const Point_2D& p1, const Point_2D& p2) const;
            /*
            ...
            const_iterator process_matching_i_pts(const Intersect_Point& matching_ip1, 
                    const const_iterator matching_ip2, const Facet_2D& facet1, 
        ...
        private:
            ...
            class Point_Find {
            public:
                Point_Find(const shared_ptr<Point_2D>& point, const Point_2D::Measurement prec);
                const bool operator()(const shared_ptr<Point_2D>& other_pt);
            private:
        ...
        public:
             ...
             */
            I_Pt_Locator(const Point_2D::Measurement prec);
            /*
             ...
             */
            const bool operator()(const Facet_2D& f1, const Facet_2D& f2, I_Pt_List& intersect_points);
        private:
            const Point_2D::Measurement precision;
            Facet_2D facet1;
            Facet_2D facet2;
            vector<shared_ptr<Point_2D>> generated_pts;
            
            const bool matches(const shared_ptr<Point_2D>& p1, const shared_ptr<Point_2D>& p2) const;
             ...
             */
            void side_i_pt_loc(const shared_ptr<Point_2D> side_start, 
             ...
             */
            const bool intersect_sides(const shared_ptr<Point_2D> f1_side_start, 
                    const shared_ptr<Point_2D> f1_side_end, const Location f1_side, 
                    const shared_ptr<Point_2D> f2_side_start, 
                    const shared_ptr<Point_2D> f2_side_end, const Location f2_side, 
                    I_Pt_Data& i_pt_data);
        ...
        public:
            ...
            typedef vector<shared_ptr<Point_2D>>::const_iterator pt_const_iterator;
            
            Facets();
            Facets(const Mesh_2D& mesh);
             ...
             */
            const shared_ptr<Point_2D> get_point(int index) const;
            /*
             * Add a Facet
             */
             ...
             */
            const bool contains(const Facet_2D& facet) const;
            /*
             * Finds the facet in the list.  Throws runtime_error if it cannot 
             * find the facet.
             */
            const Facet find_facet(const Facet_2D& facet) const;
            /*
        ...
        private:
            vector<shared_ptr<Point_2D>> point_list; // list of points
             ...
             */
            void push_back(const shared_ptr<Point_2D>& p1, const shared_ptr<Point_2D>& p2, 
                    const shared_ptr<Point_2D>& p3);
        };
	...
	/*
        ...
        private:
            ...
            struct Line_Segment {
                ...
                shared_ptr<Point_2D> point1; // the beginning of the line segment
                shared_ptr<Point_2D> point2; // the end of the line segment
                /*
                 * Constructor
                 *
                 * Arguments:
                 ...
                 */
                Line_Segment(const shared_ptr<Point_2D>& p1, const shared_ptr<Point_2D>& p2, const Location location);
                /*
	         * determines if the line segments are equal (share same points.  Location is not considered)
                 ...
                 */
                const bool shares_pt(const Line_Segment& seg, shared_ptr<Point_2D>& shared_pt) const;
            };
            ...
            public:
                 ...
                 */
                const bool contains_segment(const shared_ptr<Point_2D>& p1, const shared_ptr<Point_2D>& p2);
                // add an internal segment
                void add_internal_segment(const shared_ptr<Point_2D>& p1, const shared_ptr<Point_2D>& p2);
                // add an external segment. side_loc specifies the side of the segment on the original facet
                void add_external_segment(const shared_ptr<Point_2D>& p1, const shared_ptr<Point_2D>& p2, const Location side_loc);
                /*
                 ...
                 */
                const Line_Segment* find_segment(const shared_ptr<Point_2D>& p1, const shared_ptr<Point_2D>& p2) const;
                /*
                ...
                const Line_Segment* find_connecting_segment(const Line_Segment& segment, 
                        const Line_Segment* prev_connecting_seg, shared_ptr<Point_2D>& shared_pt, 
                ...
                const bool is_segment_external(const Line_Segment& seg3, Location& side_loc, 
                        const Facet_2D& original_facet, const vector<shared_ptr<Point_2D>>& p1p2_pts, 
                        const vector<shared_ptr<Point_2D>>& p1p3_pts, const vector<shared_ptr<Point_2D>>& p2p3_pts, 
                        const vector<shared_ptr<Point_2D>>& internal_pts) const;
                /* 
                 ...
                 */
                const bool does_seg_intersect(const Line_Segment& seg3, const Facet_2D& orig_facet, 
        ...
        public:
             ...
             */
            Facet_Builder(const bool for_f1, const Facet_2D& orig_facet, const Point_2D::Measurement prec);

            const Facet_2D& get_facet() const { return orig_facet; }
        ...
        private:
            ...
            const Point_2D::Measurement precision;
            const Facet_2D orig_facet;
            vector<shared_ptr<Point_2D>> internal_pts;
            vector<shared_ptr<Point_2D>> p1p2_pts;
            vector<shared_ptr<Point_2D>> p1p3_pts;
            vector<shared_ptr<Point_2D>> p2p3_pts;
            Segments segments;
             ...
             */
            const Line_Segment* create_link_segment(const shared_ptr<Point_2D> pt);
             ...
             */
            void gen_side_segs(vector<shared_ptr<Point_2D>>& side_points, 
                    const Location side, const shared_ptr<Point_2D>& p1, 
                    const shared_ptr<Point_2D>& p2);
             ...
             */
            const bool contains_internal_pt(const shared_ptr<Point_2D>& p1, 
                    const shared_ptr<Point_2D>& p2, const shared_ptr<Point_2D>& p3, 
                    const Facet_2D& facet);
            ...
            const Point_2D::Measurement facet_area(const Facet_2D& facet);
        };
        ...
        public:
            ...
            Facet_Sorter(const Point_2D::Measurement& prec);
            /*
            ...
            void clear();
        private:
            Point_2D::Measurement precision;
    ...
    public:
         ...
         */
        const bool operator()(const Mesh_2D& mesh1, const Mesh_2D& mesh2, Mesh_2D& mesh1_result, Mesh_2D& mesh2_result);
        /*
         ...
         */
        void difference(const Mesh_2D& mesh1, const Mesh_2D& mesh2, Mesh_2D& result);
        /* 
         ...
         */
        void intersection(const Mesh_2D& mesh1, const Mesh_2D& mesh2, Mesh_2D& result);
        /*
         ...
         */
        void merge(const Mesh_2D& mesh1, const Mesh_2D& mesh2, Mesh_2D& result);
    private:
         ...
         */
        void intersect_facets(Facets& facets1, Facets& facets2, const Point_2D::Measurement precision);
    };

}

#endif /* INTERSECT_MESHES2_2D_H */
